...
一款 Windows 原生桌面图片查看器，基于 **Qt 5.14.2**（msvc2017_64 / VS v143 工具链），界面风格对标现代看图软件：深色图标化工具栏（全部按钮为 QPainter 自绘线条图标，中文文字保留在 tooltip）+ 画布 + 可折叠缩略图侧栏。目标是"轻、快、够用"：秒开主流格式（含 HEIC），翻页/缩放/幻灯片等高频交互齐全，不做起步之外的编辑、截图等外围功能。

- 当前版本：**1.1.0**（单一来源 `src/version.h`，详见 [版本管理](#版本管理)）
...
exe 目录下 `logs\lightsee_YYYY-MM-DD.log`（spdlog daily sink：每天一个文件，零点轮转，同日重启追加写入，另有控制台 sink）。首行为版本号，便于对账；删除/解码失败等错误原因都会落盘。
